...
ScreenArea * AndromedaDistributor::next() {
    ...
    if (_dx == 0 && _dy == 0) {
        ...
        _dy = -1;
        _width = thikness;
        _height = 1.0f / areaCountOnLeftEdge();
        x = 1.0f - _width;
        y = 1.0f - _height;
        ...
        _dy = 1;
        _width = thikness;
        _height = 1.0 / areaCountOnLeftEdge();
        x = 0.0;
        y = 0.0;
...
size_t AndromedaDistributor::areaCountOnLeftEdge() const
{
    return (_areaCount - areaCountOnTopEdge() - areaCountOnBottomEdge())/2;
}

size_t AndromedaDistributor::areaCountOnRightEdge() const
{
    return areaCountOnLeftEdge();
}
